...
module.exports = {
    ...
    run: async (bot, message, args) => {
      ...
      .setColor("YELLOW");
        ...
        try {
            if (!message.member.permissions.has(Permissions.FLAGS.KICK_MEMBERS)) return message.channel.send(em1);
            if (!message.guild.me.hasPermission("KICK_MEMBERS")) return message.channel.send(em2);

            if (!args[0]) return message.channel.send('**<:error:897857752276299797> Error :- Enter A User To Kick!**\n`usage :- _kick [user]`');
